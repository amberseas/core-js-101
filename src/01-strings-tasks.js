...
function getRectangleString(width, height) {
  ...
  const middle = `\u2502${space}\u2502\n`;
  const middleAll = middle.repeat(height - 2);
  const bottom = `\u2514${dash}\u2518\n`;
  const string = top + middleAll + bottom;
  return string;
}
...
function getCardId(value) {
  ...
  if (arr.length > 2) {
    result = num.indexOf('10') + type.indexOf(arr[2]) * 13;
  } else {
    result = num.indexOf(arr[0]) + type.indexOf(arr[1]) * 13;
  }
  return result;
}
